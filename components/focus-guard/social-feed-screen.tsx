      ...
      <div className="w-full max-w-[375px] h-screen bg-background relative flex flex-col overflow-hidden">
        ...
        <main className="relative z-10 flex-1 overflow-y-auto px-5 pt-10 pb-4">
          ...
          <section className="mb-6">
            <h3 className="text-xs text-muted-foreground uppercase tracking-wider mb-3">Focusing Now</h3>
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {onlineFriends.map((friend, i) => (
                <div key={i} className="flex flex-col items-center shrink-0">
                  ...
                  <span className="text-xs text-muted-foreground mt-1.5">{friend.name}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="mb-6">
            {activities.map((activity) => (
              <div
                key={activity.id}
                className="bg-card/60 backdrop-blur-sm rounded-2xl border border-border/50 p-4 mb-3"
              >
                <div className="flex items-start gap-3">
                  {/* Avatar — always colored gradient, white text is intentional */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">{activity.activity}</p>
                    <span className="inline-block mt-2 px-2 py-0.5 rounded-full bg-amber-500/15 border border-amber-500/25 text-amber-500 dark:text-amber-400 text-xs font-medium">
                      +{activity.xp} XP
                    </span>
                  </div>
          ...
          <section className="mb-6">
            <div className="bg-primary/8 border border-primary/25 rounded-2xl p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-bold text-foreground">🏆 Weekly Challenge</h3>
                  <p className="text-sm text-muted-foreground mt-1">Focus 10 hours this week</p>
                </div>
                <button
              ...
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-muted-foreground">7.5 / 10 hours</span>
                <span className="text-xs text-muted-foreground">23 friends participating</span>
              </div>
              {onViewChallenges && (
                <button
                  onClick={onViewChallenges}
                  className="w-full text-xs text-primary font-medium hover:text-primary/80 transition-colors"
                >
                  View all challenges →
                </button>
              )}
          ...
          <section className="mb-6">
            <div className="bg-card/60 backdrop-blur-sm rounded-2xl border border-border/50 p-4">
              <h3 className="text-sm text-muted-foreground mb-2">Your Rank This Week</h3>
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-bold text-primary">#12</span>
                <span className="text-sm text-muted-foreground">out of 847 friends</span>
              </div>
              <p className="text-sm text-emerald-500 dark:text-emerald-400 mt-2">↑ 3 spots from last week</p>
            </div>
          </section>
        </main>
